...
mo.year, mo.ratingmean, x.directors, y.actors
from imdb_moviegenres mg join imdb_movies mo on mg.movieid = mo.movieid 
join (
	select replace(substring(mo.movietitle,1, length(mo.movietitle) - 7), '(', '') as title2, STRING_AGG(md.directorname, ', ') as directors
...
join(
    select replace(substring(mo.movietitle,1, length(mo.movietitle) - 7), '(', '') as title2, STRING_AGG(ma.actorname, ', ') as actors
...
group by title, mo.year , mo.movietitle, x.directors, mo.ratingmean, y.actors
